...
`personal/movies.html`, `books.html`, `music.html`, `electronics.html`, `cards.html` cross-link each other through exactly this sidebar (Movies / Music / Books / Electronics / Pokémon Cards), with the current page's link carrying `class="sidebar-link active"` — same component as Photos' album list or Blogs' "All Posts", not a separate nav row. `cards.html` nests its own pre-existing `.cards-nav` tab bar (about/collection/rare/trainers/project) inside this sidebar's main column; the two are independent navigation levels (which archive vs. which tab within Pokémon Cards).
